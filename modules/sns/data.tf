...
data "aws_iam_policy_document" "default_topic_policy" {
  dynamic "statement" {
    for_each = { for k, v in var.policy : k => v if var.create_topic == true && length(var.policy) > 0 }

    content {
      sid       = statement.value.sid
      effect    = statement.value.effect
      actions   = statement.value.actions
      resources = try(statement.value.resources, ["arn:aws:sns:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:${local.name}"])
